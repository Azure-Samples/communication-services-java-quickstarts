...
public class MediaStreaming {
  ...
  public MediaStreaming(CallConfiguration callConfiguration) {
    ...
    this.callAutomationClient =
      new CallAutomationClientBuilder()
  ...
  public void report(String incomingCallContext) {
    ...
    try {
      ...
      MediaStreamingOptions mediaStreamingOptions = new MediaStreamingOptions(
        this.callConfiguration.mediaStreamingTransportURI,
      ...
      Response<AnswerCallResult> response =
        this.callAutomationClient.answerCallWithResponse(
            answerCallOptions,
            null
          );
      ...
      Logger.logMessage(
        Logger.MessageType.INFORMATION,
        "Call Connection ID -- > " +
        answerCallResult.getCallConnectionProperties().getCallConnectionId()
      );

      registerToCallStateChangeEvent(
        answerCallResult.getCallConnectionProperties().getCallConnectionId()
      );
      ...
      Logger.logMessage(
        ...
        "Call ended unexpectedly, reason -- > " + ex.getMessage()
      );
    }
  }
  ...
  private void registerToCallStateChangeEvent(String callLegId) {
    ...
    NotificationCallback callConnectedNotificaiton =
      (
        callEvent -> {
          Logger.logMessage(
            Logger.MessageType.INFORMATION,
          ...
          EventDispatcher
            .getInstance()
            .unsubscribe(CallConnectedEvent.class.getName(), callLegId);
        }
      );

    NotificationCallback callDisconnectedNotificaiton =
      (
        callEvent -> {
          EventDispatcher
            .getInstance()
            .unsubscribe(CallDisconnectedEvent.class.getName(), callLegId);
    ...
    EventDispatcher
      .getInstance()
      .subscribe(
        CallConnectedEvent.class.getName(),
        callLegId,
        callConnectedNotificaiton
      );
    EventDispatcher
      .getInstance()
      .subscribe(
        CallDisconnectedEvent.class.getName(),
        callLegId,
  ...
  public String getResponse(Response<?> response) {
    ...
    responseString =
      new StringBuilder(
        "StatusCode: " + response.getStatusCode() + ", Headers: { "
      );
