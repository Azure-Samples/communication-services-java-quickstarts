...
export const utils = {
    ...
    startRecording: async (id: string): Promise<RecordingApiResponse> => {
        const response = await fetch('/startRecording?serverCallId=' + id);
        if (response.ok) {
            var recordingid = await response.json();
            return { recordingId: recordingid.recordingId, message: '' };
        }
    ...
    stopRecording: async (serverCallId: string, recordingId: string): Promise<RecordingActionResponse> => {
        const response = await fetch('/stopRecording?serverCallId=' + serverCallId + '&recordingId=' + recordingId);
        if (response.ok) {
            return { message: '' };
        }
