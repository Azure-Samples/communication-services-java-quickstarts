...
public class ProgramSample {
    ...
    public  RoomsClient createRoomsClientWithConnectionString() {
        RoomsClient roomsClient = new RoomsClientBuilder().connectionString(appConfig.getConnectionString()).buildClient();
    ...
    public  CommunicationIdentityClientBuilder getCommunicationIdentityClientBuilder() {
        ...
        CommunicationConnectionString connectionStringObject = new CommunicationConnectionString(appConfig.getConnectionString());
    ...
    tags = {"5. Test"})
    public String TeastApi() {
        return "Hello, World! its working";
    }
    @PostMapping("/create-room")
    @Operation(summary = "Create a new communication room", description = "Creates a new room and returns the room ID along with user tokens.", 
    tags = {"1. Create Room"})
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Room created successfully."),
        @ApiResponse(responseCode = "500", description = "Error occurred while creating the room.")
    })
        ...
        try {
            ...
            if (roomResult != null) {
                ...
                var responsePayload = Map.of(
                        ...
                        "validFrom", roomResult.getValidFrom(),
                        "validUntil", roomResult.getValidUntil(),
                        "tokens", List.of(
                                Map.of(
                                        "User1 id:", user1.getId(),
                                        "User1 token value:", userToken1.getToken(),
                                        "expiresAt:", userToken1.getExpiresAt(),
                                        "role:", ParticipantRole.PRESENTER
                                ),
                                Map.of(
                                        "User2 id:", user2.getId(),
                                        "User2 token value:", userToken2.getToken(),
                                        "expiresAt:", userToken2.getExpiresAt(),
    ...
    @GetMapping("/connectCall")
    @Operation(summary = "Connect the call", description = "Connects a call to the communication room.",
    tags = {"2. Call Connection"})
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Call connected successfully."),
        @ApiResponse(responseCode = "400", description = "Invalid room ID or connection parameters."),
        @ApiResponse(responseCode = "500", description = "Error occurred while connecting the call.")
    })
        ...
        try {
                ...
                ConnectCallOptions connectCallOptions = new ConnectCallOptions(callLocator, callbackUri)
                    ...
                    .setCallIntelligenceOptions(
                        new CallIntelligenceOptions().setCognitiveServicesEndpoint(appConfig.getCognitiveServiceEndpoint())
                    );
    ...
    public ResponseEntity<String> handleCallbacks(@RequestBody String requestBody) {
        try {
            ...
            for (CallAutomationEventBase event : events) {
                ...
                // Assuming asyncClient is an instance of your client class that interacts with the API
    ...
    @Operation(summary = "Add PSTN Participant to the call", description = "Adds a PSTN participant to an active call"
    ,tags = {"3. Add Participants"})
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "PSTN participant added successfully."),
        @ApiResponse(responseCode = "400", description = "Invalid participant information."),
        @ApiResponse(responseCode = "500", description = "Error occurred while adding the PSTN participant.")
    })
    public ResponseEntity<String> addPstnParticipant() {
        ...
        try {
            ...
            getCallConnection()
                ...
                .subscribe(response -> {
                    log.info("Added PSTN participant with invitation ID: {}", response.getValue().getInvitationId());
                });
    ...
    @PostMapping("/hangUp")
    @Operation(summary = "Hang up the call", description =   "Ends the active call for all participants.",
    tags = {"4. Call Termination"})
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Call terminated successfully."),
        @ApiResponse(responseCode = "400", description = "Invalid call connection ID."),
        @ApiResponse(responseCode = "500", description = "Error occurred while terminating the call.")
    })
    public Mono<ResponseEntity<String>> hangUp() {
        try {
            ...
            return getCallConnection() // Initiate the hang-up process
                ...
                .onErrorResume(error -> {
                    log.error("Error when terminating the call for all participants: {} {}", error.getMessage(), error.getCause());
                    return Mono.just(ResponseEntity.status(500).body("Error terminating the call: " + error.getMessage()));  // Handle error response
                });
        } catch (Exception e) {
